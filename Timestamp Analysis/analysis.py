...
# BEG printed in osi_crypto_provider.c ln 523 after line "if (*one_time_pad == NULL || signal_buffer_len(*one_time_pad) < plaintext_len) {"
# END printed in osi_crypto_provider.c ln 572 at end of if statement to extend one-time pad	
...
# OVERLAY - Time from when the user initiates decryption till plaintext appears in gui
# REC printed in osi.c ln 586 after line "num_epoll_events = epoll_wait(osi->epoll_fd, epoll_events, NUM_EVENT_TYPES, -1);"
# CFM printed in osi.c at beginning of function osi_perform_decryption_session()
# END printed in osi_gui_base.py at end of method set_text()
def time_overlay_decrypt_to_gui(timestamp_list):
	# Correctly order REC and CFM when they have the same timestamp
	reorder_ties_given_priority(timestamp_list, "REC", "CFM")			
	...
	print("Average time from ctrl+alt+u to plaintext in GUI: " + str(get_average(duration_list)))
	
# BEG printed in osi_gui_encrypt.py at beginning of function finish_encryption()
# END printed in osi_kbd.c at end of function osi_kbd_force_paste()
def time_overlay_encrypt_to_destination(timestamp_list):
	# Correctly order BEC and END when they have the same timestamp
	reorder_ties_given_priority(timestamp_list, "BEG", "END")
...
def main():
	...
	#time_overlay_decrypt_to_gui(timestamp_list)
	#time_overlay_encrypt_to_destination(timestamp_list)
